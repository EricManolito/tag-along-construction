...
- ssh into the server
    - For Mac and Linux: `ssh root@tagalongconstruction.com`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo emanolito`
    - replace emanolito with your own
- create password
    - `passwd emanolito`
- Move .ssh key file
    - `cp -r .ssh /home/emanolito`
- change ownership of the .ssh directory
    - `cd /home/emanolito`
    - `chown -R emanolito:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh emanolito@tagalongconstruction.com`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- reboot
    - `sudo reboot`
- ssh back in
- `ssh emanolito@tagalongconstruction.com`
...
- create a directory for your deployment
    - `sudo mkdir /var/www/tagalongconstruction.com/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/tagalongconstruction.com/html`
    - create a deployment in Webstorm and upload it to the server // do further down on this list of codes


## create a new nginx block
- Create a new nginx block for your domain in the sites-available directory
- `sudo vim /etc/nginx/sites-available/tagalongconstruction.com`
...
server {
    ...
    root /var/www/tagalongconstruction.com/html;
    index index.html index.htm index.nginx-debian.html;

    server_name tagalongconstruction.com www.tagalongconstruction.com;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/tagalongconstruction.com /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/emanolito/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/emanolito/.ssh/github_deploy_key.pub >> /home/emanolito/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/emanolito/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H tagalongconstruction.com >> ~/.ssh/known_hosts 
         scp -r ./dist/* emanolito@tagalongconstruction.com:/var/www/tagalongconstruction.com/html
```
